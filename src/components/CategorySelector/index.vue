  ...
  <div>
    <el-form :inline="true" class="demo-form-inline" :model="cForm" :disabled="!isShowList">
      ...
      <el-form-item label="三级分类">
        <el-select v-model="cForm.category3Id" value="" placeholder="请选择" @change="handlerCategory3(cForm.category3Id)">
          <el-option
...
export default {
  ...
  methods: {
    ...
    async handleCategory1() {
      ...
      const result = await this.$API.category.getCategory2(
    ...
    async handleCategory2() {
      ...
      const result = await this.$API.category.getCategory3(
        this.cForm.category2Id
      )
      if (result.code === 200) {
